...
function Auth() {
    ...
    const user ={
        name: "Sultan Bozkurt",
        avatar: "https://i.scdn.co/image/ab6775700000ee857a6ed57b1044cfab600da10f"
    }
  return (
...
{({open}) => (
    ...
    <Menu.Items className={"absolute p-1  top-full right-0 w-48 bg-active rounded translate-y-2"}>
      <Menu.Item>
        {({ active }) => (
          <a
            className={`h-10 items-center flex justify-between px-2 text-sm rounded ${active && 'bg-white bg-opacity-10'}`}
            href="#"
          >
            Account 
      ...
      <Menu.Item>
        {({ active }) => (
          <a
            className={`h-10 items-center flex px-2 text-sm rounded ${active && 'bg-white bg-opacity-10'}`}
            href="#"
          >
            Profile 
          </a>
